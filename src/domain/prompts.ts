...
function context(input: WorkspacePromptInput): TopicContext {
  ...
  if (input.topic === 'creation' || input.topic === 'clip') {
    ...
    const guidance = [
      ...
      'MANDATORY for every audio/video edit: read its adjacent .vandashi.json analysis first. Speech assets contain source-timed transcription.segments and, when aligned, transcription.words; music/effects are explicitly exempt. Use this evidence to select source ranges, synchronize images to spoken ideas, and align requested captions or word highlights with actual speech. Preserve the distinction between source timestamps and composition/clip time after trims or speed changes. Never invent word timing when only segment timing is available; disclose missing or uncertain evidence.',
      input.mode === 'edit'
...
export function buildWorkspacePrompt(input: WorkspacePromptInput): string {
  ...
  const sections = [
    ...
    input.transcriptionGuidePath
      ? `- MANDATORY BEFORE ADDING, DOWNLOADING, GENERATING, OR REPLACING ANY AUDIO/VIDEO ASSET: read the app-owned transcription guide at ${quote(input.transcriptionGuidePath)} and run its exact command on the final asset path. This applies to attachments and every chat, not just the asset page. The command uses the SAME classification/transcription process as Vandashi. Verify the persisted category and source hash; dialog requires a completed full-file transcript, while music/effects require their explicit exemption. Do not hand-author or invent transcripts. These timestamps are REQUIRED for subsequent video editing. In your final response identify whether the added media is audio or video and report its category and verified transcription status. In read mode, do not run the mutating command; ask the creator to switch to edit mode.`
      : '- Audio/video asset imports require Vandashi transcription metadata. If the app-owned transcription guide is unavailable, report that preparation is unavailable; never invent a transcript or claim metadata verification succeeded.',
    repository
