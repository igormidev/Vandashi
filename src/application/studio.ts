...
Update ONLY script.md. Describe the meaningful changes at the affected scenes: displayed words, typography, colors, sizes, layout and crop, element layering, entrance/exit motion, transitions, scene duration and timing, audio clips/levels/fades, and asset substitutions as supported by the actual change. Keep the existing narration and untouched scene directions. Retain usable exact timestamps when present. Every referenced image, video, audio or sound effect must use the application's mention syntax @[asset name](absolute filesystem path), resolved from the real project/video_assets and shared asset files; do not invent files or references. If a document is initially blank, write a concise scene-by-scene account of the actual composition, rather than inventing a new story.
