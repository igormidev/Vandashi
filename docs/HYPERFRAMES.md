...
`thumbnails/` is the release-artwork directory; composition images belong in `video_assets/`. Unreferenced release thumbnails do not invalidate static compositions or literal GSAP timelines. The fingerprint conservatively includes the entire thumbnail directory when source references a candidate, contains unknown/dynamic JavaScript, uses an embedded document or unclassified resource loader, or cannot be inspected confidently. Thus arbitrary generated code may still require a new render after changing a thumbnail. The classifier recognizes only literal GSAP operations and the exact pinned GSAP 3.15.0 library hash; upgrading or modifying that library falls back to including thumbnails until reviewed. Actual thumbnail references, encoded paths, dynamic code, library modifications, and a real imported-thumbnail storage flow are covered by `render-fingerprint.test.ts` and `storage-thumbnails.test.ts`.
