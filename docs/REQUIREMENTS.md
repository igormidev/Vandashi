...
| ASSET-04/07/10/13 (identity and concurrent edits)  | `tests/storage-asset-revisions.test.ts` verifies duplicate detection for original and embedded bytes, invalidation after external media changes, exact media/sidecar revisions, stale-save rejection before any Git commit, conflict detection during held metadata preparation, cleanup, and fresh-revision retry. `docs/ASSET-METADATA.md` documents the contract and the external-writer race limit. New shared/video inspector conflict-and-reset native scenarios await their coordinated run.                                     |
