...
Prepared review fields are locked alongside the editable chat request. **Edit review** hides the chat while the user changes the review; preparing again carries those changes into a new request. An independently edited chat draft is preserved and requires an explicit choice between that draft and the new prepared request before it can be sent. A finished independent clip remains accessible for editing and publishing when the parent video needs a new render; creating another clip and publishing the parent still require a fresh parent render.
